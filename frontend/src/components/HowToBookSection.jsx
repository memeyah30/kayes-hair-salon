...
const HowToBookSection = () => {
  ...
  return (
    <section
      id="how-to-book"
      className="home-section px-4 py-8 md:px-8 md:py-10 bg-[linear-gradient(180deg,#f7f2ff_0%,#efe6ff_52%,#f7f2ff_100%)]"
    >
          ...
          >
            {bookingSteps.map((item, index) => (
              <motion.article
                key={item.step}
                variants={fadeUp}
                className="group relative overflow-hidden rounded-[20px] border border-[#dcccff] bg-white/88 p-3.5 md:p-4 shadow-[0_10px_24px_rgba(71,46,137,0.08)] backdrop-blur-sm transition duration-300 hover:-translate-y-1 hover:border-[#c6b3ff] hover:shadow-[0_16px_30px_rgba(71,46,137,0.14)]"
              >
                ...
                <div className="flex items-start justify-between gap-4">
                  <span className="inline-flex rounded-full bg-[#f1ebff] px-2.5 py-1 text-[10px] font-semibold tracking-[0.18em] text-[#6c55c5]">
                    STEP {item.step}
                  </span>
                  <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-2xl bg-[linear-gradient(135deg,#785fff_0%,#5740d8_100%)] text-white shadow-[0_10px_18px_rgba(66,42,150,0.22)] transition duration-300 group-hover:scale-105 group-hover:shadow-[0_14px_24px_rgba(66,42,150,0.28)] [&>svg]:h-4.5 [&>svg]:w-4.5">
                    {item.icon}
                  </span>
                </div>
