        ...
        <nav className="flex w-full flex-1 overflow-y-auto flex-col items-center gap-2 pt-1 xl:gap-2.5 no-scrollbar">
          {links.map((link) => {
            if (link.children) {
              ...
              return (
                <button
                  key={`collapsed-${link.label}`}
                  onClick={() => setIsOpen(true)} // Open sidebar if clicked while collapsed
                  className={`flex h-10 w-10 items-center justify-center rounded-2xl transition xl:h-11 xl:w-11 ${
                    isAnyChildActive
              ...
              return (
                <div key={link.label} className="space-y-1">
                  <button
                    ...
                    className={`w-full flex items-center justify-between gap-3 rounded-xl px-3 py-2 transition ${
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-white/12 border border-white/20">
                        {renderIcon(link.icon)}
                      </span>
                  ...
                  {isExpanded && (
                    <div className="ml-4 space-y-1 pl-4 border-l border-white/10 animate-fadeIn">
                      {link.children.map((child) => (
                        <NavLink
                          ...
                          onClick={(event) => handleSidebarLinkClick(event, child, true)}
                        >
                          {child.label}
                        </NavLink>
                      ))}
